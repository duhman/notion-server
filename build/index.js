...
const toolHandlers = {
    ...
    async read_page(args) {
        ...
        try {
            ...
            for (const block of blocksResponse.results) {
                ...
                const textContent = block[type]?.rich_text?.map((text) => text.plain_text).join("") || "";
    ...
    async create_page(args) {
        const { title, content, parentPageId, properties } = args;
        try {
    ...
    async update_page(args) {
        ...
        try {
            ...
            const createBlocks = (content, _type) => {
                const lines = content.split('\n');
                const blocks = [];
                let currentCodeBlock = null;
                for (let i = 0; i < lines.length; i++) {
    ...
    async retrieve_comments(args) {
        const { pageId, startCursor, pageSize } = args;
        try {
    ...
    async create_database(args) {
        const { parentPageId, title, properties } = args;
        try {
    ...
    async query_database(args) {
        const { databaseId, filter, sort } = args;
        try {
    ...
    async update_block(args) {
        const { blockId, content, type = "paragraph" } = args;
        try {
    ...
    async update_database_item(args) {
        const { pageId, properties } = args;
        try {
    ...
    async update_database(args) {
        const { databaseId, title, description, properties } = args;
        try {
